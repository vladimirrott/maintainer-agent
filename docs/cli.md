...
| `maintainer release <issue> <user> <reason>` | retract an offer so `offers` stops counting it. It posts a comment carrying the marker `offers` reads, names that one person and refuses a reason naming a second, and reads the comment back before reporting success. Prose alone does not retract anything: two offers withdrawn in public on 2026-09-22 still read as live holds, and the campaign that followed was told to hand work to the contributor those phantom holds ruled out. At `POST=off` it rehearses |
